...
export default function Particles({
  ...
  const circleParams = (): any => {
    ...
    const pX = 0;
    const pY = 0;
    const size = Math.floor(Math.random() * 2.5) + 0.3;
    const targetAlpha = parseFloat((Math.random() * 0.7 + 0.15).toFixed(1));
  ...
  const remapValue = (
    ...
    const rel1 = stop1 - start1;
    const rel2 = stop2 - start2;
    return start2 + (rel2 * (value - start1)) / rel1;
  };
